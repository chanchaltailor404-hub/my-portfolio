        ...
        <div className="flex items-center space-x-2.5">
          {authToken ? (
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.03 }}
              className="group flex items-center space-x-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full px-3.5 py-1.5 text-xs text-white/70 font-mono tracking-wider focus:outline-none cursor-pointer"
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Persistent high-fidelity Coming Soon Card */}
          <motion.div variants={cardItemVariants} className="h-full">
            <motion.div
              whileHover={{ y: -6, scale: 1.015 }}
              transition={{ duration: 0.25, ease: "easeOut" }}
              className="group p-5 bg-black/45 backdrop-blur-md hover:bg-black/60 rounded-2xl border border-dashed border-white/20 hover:border-pink-500/30 transition-all duration-300 flex flex-col justify-between h-full cursor-pointer relative overflow-hidden"
            >
              {/* Subtle backglow effect on hover */}
              <div className="absolute inset-0 bg-gradient-to-br from-pink-500/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none select-none z-0" />
              <div className="glitch-scanline" />

              <div className="flex-1 z-10 w-full flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center text-[10px] font-mono text-[#F094E6]/70 mb-5">
                    <span className="tracking-widest uppercase">// PROJECT COMING SOON</span>
                    <div className="flex items-center space-x-1.5 bg-pink-500/10 border border-pink-500/20 px-2 py-0.5 rounded">
                      <span className="w-1.5 h-1.5 rounded-full bg-pink-400 animate-ping" />
                      <span className="font-mono text-[9px] text-[#F094E6] uppercase tracking-wider">ACTIVE</span>
                    </div>
                  </div>

                  {/* Interactive glowing cyber sphere */}
                  <div className="w-full h-36 border border-white/5 bg-zinc-950/40 rounded-xl mb-4 flex flex-col items-center justify-center relative overflow-hidden group-hover:border-pink-500/15 transition-colors">
                    <div className="absolute inset-0 bg-radial-gradient from-pink-500/10 to-transparent pointer-events-none opacity-50 group-hover:opacity-100 transition-opacity" />
                    
                    <div className="relative flex items-center justify-center">
                      <motion.div 
                        animate={{ rotate: 360 }}
                        transition={{ repeat: Infinity, duration: 20, ease: "linear" }}
                        className="w-14 h-14 rounded-full border border-dashed border-pink-500/20 flex items-center justify-center"
                      />
                      <div className="absolute w-8 h-8 rounded-full bg-pink-500/10 border border-pink-400/20 flex items-center justify-center">
                        <Sparkles className="w-3.5 h-3.5 text-pink-300 animate-pulse" />
                      </div>
                    </div>
                    
                    <span className="text-[10px] uppercase font-mono text-white/30 tracking-widest mt-4 group-hover:text-pink-300/40 transition-colors">
                      [ PIPELINE_ONLINE ]
                    </span>
                  </div>
                  ...
                  <p className="text-xs text-white/70 font-light leading-relaxed font-sans mb-4">
                ...
                <div className="mt-auto space-y-4">
                  <p className="text-[11px] text-white/40 font-mono italic">
                    Check back soon or follow my github!
                  </p>
                  
                  {/* CTA button to follow Github */}
                  <div className="pt-3 border-t border-white/5">
                    <a
                      href={socialGithub.startsWith("http") ? socialGithub : `https://github.com/${socialGithub}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full inline-flex items-center justify-center space-x-2 px-4 py-2 text-xs font-mono font-semibold rounded-lg bg-white/5 hover:bg-white/10 text-white border border-white/10 hover:border-pink-500/30 transition-all cursor-pointer"
                    >
